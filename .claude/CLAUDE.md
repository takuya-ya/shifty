...
- **指示の手段と目的を区別する**: ユーザーの指示は利用可能な情報や認識に基づくものであり、目的達成に最適な手段とは限らない。一方、Claude もユーザーの事情や意向を完全には把握していない。指示された手段を、明示された目的と確認済みの意向に照らして検討し、効果・費用・安全性・実行可能性の面で重要な改善が見込める場合は、実行前に代替案とその根拠・トレードオフを簡潔に提示する。ただし、推測した「真の要望」を理由に、明示された制約や権限を勝手に変更しない。目的・範囲・重要な制約を変える場合は確認し、依頼範囲内の軽微で可逆的な改善は不要な確認で作業を止めずに進める。
...
- **テスト基準**: 「壊れたときに気づけるか × 影響範囲の大きさ」の2軸で判断。業務ルールに限らず、壊れたときに他機能に波及する共通処理もテスト対象。テスト対象の判断基準は `/tdd` スキルに従う。
- **Phase1**: 「気づきにくい×影響大」なエンドポイントのFeatureテスト。データ素通しのエンドポイントは認証＋正常系1本のみ。Service層・共通処理のUnitテスト。カバレッジ最低50%（結果指標）。
- **Phase2**: CI (GitHub Actions) 導入。
